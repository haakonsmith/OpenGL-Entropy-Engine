...
void m_2dRenderer::genVertexBuffer(Renderable *_renderable)
{
  ...
  glBufferData(
      GL_ARRAY_BUFFER,
      _renderable->Vertices.size() * sizeof(Vertex),
...
void m_2dRenderer::buffer(Renderable *_renderable)
{
  ...
  if (_renderable->shader == nullptr)
    _renderable->shader = program;
...
void m_2dRenderer::render(Renderable *_renderable)
{
  ...
  GL_LOG("Render");
  // Draw the triangle !
  glDrawArrays(GL_TRIANGLES, 0, _renderable->Vertices.size()); // Starting from vertex 0; 3 Vertices total . 1 RightTriangle
...
void m_2dRenderer::renderLine(const vec3 &p1, const vec3 &p2)
{
  ...
  Vertex verts[] = {
      Vertex(modelSpace(p1)),
      Vertex(modelSpace(p2))};
...
void m_2dRenderer::renderOutline(const Renderable &_renderable)
{
  ...
  glDrawArrays(GL_LINE_LOOP, 0, _renderable.Vertices.size()); // Starting from vertex 0; 3 Vertices total . 1 RightTriangle
  GL_LOG("draw arrays ");
...
m_2dRenderer::m_2dRenderer(unsigned int width, unsigned int height)
{
  ...
  debugShader = shared_ptr<Shader>(new Shader("shaders/debug/red.vertexshader", "shaders/debug/red.fragmentshader"));
  debugLineShader = shared_ptr<Shader>(new Shader("shaders/debug/line.vertexshader", "shaders/debug/line.fragmentshader"));

  program = shared_ptr<Shader>(new Shader("shaders/SimpleVertexShader.vertexshader", "shaders/SimpleFragmentShader.fragmentshader"));
  // instanceShader = shared_ptr<Shader>(new Shader("shaders/Instance.vertexshader", "shaders/Instance.fragmentshader"));
